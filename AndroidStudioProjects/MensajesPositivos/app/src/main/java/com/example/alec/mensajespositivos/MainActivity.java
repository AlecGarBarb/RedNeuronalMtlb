package com.example.alec.mensajespositivos;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        constraintLayout = (ConstraintLayout) findViewById(R.id.fondo);
        constraintLayout.setBackgroundColor(Color.rgb(100,100,100));

        textView = findViewById(R.id.mensaje);

        textView.setText("Qué onda, me paso de lanza de guapo.");

        editText = (EditText) findViewById(R.id.editText);

        Toast.makeText(this,"HARD CODE TEST" , Toast.LENGTH_LONG).show();
        Log.i(TAG, "Welcome to the Jungle BABY!");
    ...
    public void anterior(View view){
        if(random  > 0 ){
            ...
            random = mensajes.length-1;
        }
        textView.setText(mensajes[random]);
    }

    public void elegido(View view){

        String valor= editText.getText().toString();
        int randomsito=Integer.parseInt(valor);

        if(randomsito  >= 1 && randomsito < mensajes.length){
            random = randomsito -1;
            textView.setText(mensajes[random]);
        }else{
            Toast.makeText(this,"Número no válido, intente otro." , Toast.LENGTH_LONG).show();
        }
